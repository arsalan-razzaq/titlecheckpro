...
import { DetectedCountryCta } from "./detected-country-cta";
...
export function HomeSections() {
  return <>
    ...
    <section className="section"><div className="container"><SectionHeading title="How It Works" description="Four straightforward steps from detected location to report delivery." /><div className="grid gap-5 md:grid-cols-4">{[["01", MapPinned, "Detect your location"], ["02", FileSearch, "Enter vehicle details"], ["03", ClipboardCheck, "Choose a report package"], ["04", Mail, "Receive your vehicle report"]].map(([num, Icon, title]) => { const C = Icon as typeof MapPinned; return <article key={String(num)} className="relative rounded-2xl border border-[#E5E5E5] p-6"><span className="font-heading text-4xl font-black text-[#FFC400]">{String(num)}</span><C className="mt-8 size-8" /><h3 className="mt-4 font-heading text-lg font-black">{String(title)}</h3></article>; })}</div></div></section>
    <section className="section bg-[#F5F5F5]"><div className="container"><SectionHeading title="Your Report Location" description="The report form is selected automatically from the visitor location. Other countries continue with USD checkout." /><DetectedCountryCta /></div></section>
