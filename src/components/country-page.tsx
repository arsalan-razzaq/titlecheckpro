...
export function countryMetadata(countryKey: CountryKey): Metadata {
  ...
  return {
    ...
    description: `Request a ${country.name}-specific vehicle history report with local vehicle labels and ${country.currency} pricing.`,
...
export function CountryPage({ countryKey }: { countryKey: CountryKey }) {
  const country = countries[countryKey];
  return <>
    <PageHero eyebrow={`${country.flag} ${country.currency} report requests`} title={`${country.name} Vehicle History Reports`} description={`Enter vehicle details using labels configured for ${country.name}, compare packages and submit a secure report enquiry.`} />
    <section className="section bg-[#F5F5F5]"><div className="container"><SectionHeading title="Choose a report package" description={`All prices are centrally configured in ${country.currency}. Final pricing will appear as soon as it is supplied.`} /><PricingGrid countryKey={countryKey} /></div></section>
