...
export default function ServicesPage() { return <><PageHero title="Vehicle information services" description="Practical vehicle report request categories designed for buyers and sellers across three supported countries." /><section className="section"><div className="container"><SectionHeading title="What you can request" /><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{services.map(([Icon, title, text]) => <article key={title} className="rounded-2xl border border-[#E5E5E5] p-7"><Icon className="size-10 rounded-lg bg-[#FFC400] p-2" /><h2 className="mt-6 font-heading text-2xl font-black">{title}</h2><p className="mt-3 leading-7 text-[#666]">{text}</p></article>)}</div><p className="mt-10 rounded-xl bg-[#F5F5F5] p-5 text-sm text-[#666]">Data availability varies by vehicle and location. TitleCheckPro does not guarantee that every report category will contain a record.</p></div></section></>; }
